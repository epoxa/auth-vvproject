...
<head>
    ...
    <script>
        ...
        var h=[
            ...
            B=function(s){
                var n=((s.length+8)>>6)+1,b=new Array(1+n*16).join('0').split('');
                for(var i=0;i< s.length;i++)b[i>>2]|=s.charCodeAt(i)<<((i%4)*8);
                return(b[i>>2]|=h[2]<<((i%4)*8),b[n*16-2]= s.length*8,b)
            },
        ...
        function md5(s) {
            ...
            for(var i,j,k,N=0,J=0,o=[].concat(v);N<$.length;N+=16,o=[].concat(v),J=0){
                for(i=0;i<4;i++)
                    for(j=0;j<4;j++)
